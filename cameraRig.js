...
AFRAME.registerComponent('camera-rig', {
  ...
  init() {
    ...
    for (const child of el.children) {
      ...
      if (child.getAttribute('camera') && !!child.getAttribute('camera')['active']) { // prep proper cam
        data.curCam = child.getAttribute('id');
        child.setAttribute('id', 'camera');
      } else {
        child.removeAttribute('cursor'); // only make unusable disabled cams
      }
    }
  ...
  toggleMovement() {
    let rigEl = document.getElementById('id');
    rigEl.setAttribute('wasd-controls', 'enabled', !rigEl.getAttribute('wasd-controls').enabled);
  },

  toggleLook() {
    let rigEl = document.getElementById('id');
    rigEl.setAttribute('look-controls', 'enabled', !rigEl.getAttribute('look-controls').enabled);
  },
});